...
// Static swatch colours that visually hint at each filter without needing a live video frame
const FILTER_SWATCHES = {
  normal:  ["#f8f8f8", "#c0c0c0"],
  warm:    ["#f5a623", "#e8602c"],
  cool:    ["#4fc3f7", "#1565c0"],
  mono:    ["#aaaaaa", "#333333"],
  vivid:   ["#ff4081", "#7c4dff"],
...
function FilterCarousel({ filters, activeId, onSelect, videoRef, isNative }) {
  const previewRefs = useRef({});

  useEffect(() => {
    if (isNative) return; // native has no video element to read from
    filters.forEach(f => {
          ...
          <button key={filter.id} onClick={() => onSelect(filter.id)}
            ...
            <div style={{
              ...
              <div style={{
                ...
                {isNative ? (
                  // Static gradient swatch — no live video available on native
                  <div style={{ width:"100%", height:"100%",
                    background: `linear-gradient(135deg, ${(FILTER_SWATCHES[filter.id]||FILTER_SWATCHES.normal)[0]}, ${(FILTER_SWATCHES[filter.id]||FILTER_SWATCHES.normal)[1]})`,
...
function SplashScreen({ state }) {
  // Covers the transparent WebView with black until the native camera is live
  const visible = state !== CAM_STATE.GRANTED;
  return (
        ...
        {snaps.map(snap => (
          <div key={snap.id} style={{ aspectRatio:"1", borderRadius:6, overflow:"hidden", position:"relative" }}>
            ...
            <button onClick={() => {
              ...
              a.href = snap.media;
              a.download = `snip_${snap.id}.${snap.type === "video" ? "mp4" : "png"}`;
              a.click();
...
function EditScreen({ media, mediaType, onDone, onDiscard, toast }) {
  ...
  useEffect(() => {
    ...
    if (mediaType === "photo") {
      ...
      // For video: size the overlay canvas to match display size
      // so paint/text tools work in correct coordinates
      const syncSize = () => {
      ...
      const vid = document.getElementById("edit-video");
      if (vid) { vid.addEventListener("loadedmetadata", syncSize, { once: true }); syncSize(); }
    }
  }, [media, mediaType]);
  ...
  const handlePaintStart = (e) => {
    ...
    pushHistory();
    if (canvasRef.current) canvasRef.current.dataset.painted = "1";
    setPainting(true);
  ...
  return (
    <div style={{ position:"absolute", inset:0, zIndex:55, background:"#000",
      ...
      <div ref={wrapRef} style={{ flex:1, position:"relative", overflow:"hidden",
        ...
        {mediaType === "video"
          ? <>
              <video ref={el => { if (el) el.src = window.Capacitor?.convertFileSrc
                  ? (media.startsWith("file://") || media.startsWith("/")
                      ? window.Capacitor.convertFileSrc(media.startsWith("file://") ? media : "file://" + media)
                      : media)
                  : media; }}
                id="edit-video" style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }}
...
export default function SnipChat() {
  ...
  const startCamera = useCallback(async (mode) => {
    ...
    try {
      if (isNative.current) {
        // Trigger MICROPHONE permission dialog via audio-only getUserMedia.
        // This works in Capacitor WebView for audio even though video is blocked.
        // We stop the stream immediately — we only need the permission grant.
        try {
          const micStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
          micStream.getTracks().forEach(t => t.stop());
        } catch (_) {} // if denied or unavailable, keep going — CameraPreview handles camera

        // Trigger CAMERA permission dialog via CameraPreview.start()
  ...
  // Decodes a JPEG base64 from CameraPreview, fixes rotation, mirrors front cam,
  // applies CSS filter, and returns a portrait PNG data URL.
  const processNativePhoto = (base64jpeg, filterCss, isFront) => new Promise(resolve => {
    const img = new Image();
    img.onload = () => {
      const iw = img.naturalWidth, ih = img.naturalHeight;
      const isLandscape = iw > ih;
      // Output canvas is always portrait
      const outW = isLandscape ? ih : iw;
      const outH = isLandscape ? iw : ih;
      const c = document.createElement("canvas");
      ...
      ctx.save();
      if (isLandscape) {
        // Rotate 90° CW: move origin to right edge then rotate
        ctx.translate(outW, 0);
        ctx.rotate(Math.PI / 2);
        // After rotation the image draws at (0,0) with original dimensions
      }
      if (isFront) {
        // Mirror: flip horizontally within the (possibly rotated) space
        const drawW = isLandscape ? ih : iw;
  ...
  const handleCapture = async () => {
    ...
    if (isNative.current) {
      ...
      const png = await processNativePhoto(result.value, filterCss, facingMode === "user");
      setCaptured({ media: png, type: "photo" });
    } else {
      const canvas = canvasRef.current;
      if (!canvas) return;
  ...
  const startRecording = useCallback(async () => {
    ...
    if (isNative.current) {
      // Request mic permission via CameraPreview before recording starts.
      // requestPermissions() is part of the plugin and covers RECORD_AUDIO on Android.
      try { await CameraPreview.requestPermissions(); } catch (_) {}
      try {
        await CameraPreview.startRecordVideo({});
      } catch (e) { console.error("startRecordVideo failed", e); }
    } else {
      // Browser: MediaRecorder on the getUserMedia stream
      ...
      mr.onstop = () => {
        ...
        setCaptured({ media: URL.createObjectURL(blob), type: "video" });
      };
      mr.start(100);
  ...
  const stopRecording = useCallback(async () => {
    ...
    if (isNative.current) {
      try {
        ...
        // result.videoFilePath is a file:// URI on device
        setCaptured({ media: result.videoFilePath, type: "video" });
        toast("Video captured!", { icon: "🎥" });
      } catch (e) {
        console.error("stopRecordVideo failed", e);
    ...
    try {
      ...
      if (isNative.current) {
        if (type === "video" && (media.startsWith("file://") || media.startsWith("/"))) {
          // Already a native file path from CameraPreview — move into Documents
          try {
            ...
            // If copy fails just keep the original path
            savedMediaPath = media;
          }
        } else {
          // Photo (base64 data URL) — write to Documents
        ...
        // Browser: convert blob URL to base64 for localStorage
        if (type === "video" && media.startsWith("blob:")) {
          const blob = await (await fetch(media)).blob();
          savedMediaPath = await new Promise(resolve => {
      ...
      const snap = { id, media: savedMediaPath, type };
      setSavedSnaps(prev => {
  ...
  const downloadSnap = (media, type) => {
    const a = document.createElement("a");
    a.href = media;
    a.download = `snip_${Date.now()}.${type === "video" ? "mp4" : "png"}`;
    a.click();
      ...
      <div style={{ width:"100%", maxWidth:450, margin:"0 auto", height:"100dvh",
        ...
        <div style={{ position:"absolute", top:0, left:0, right:0, padding:"12px 16px",
          ...
          <div style={{ display:"flex", gap:14, alignItems:"center" }}>
            ...
            <button onClick={toggleFlash} style={{ background:"none", border:"none", cursor:"pointer",
              display:"flex", flexDirection:"column", alignItems:"center", gap:2 }}>
              <Zap size={22}
                color={flashMode === "off" ? "rgba(255,255,255,0.5)" : BRAND.yellow}
                fill={flashMode === "on" || flashMode === "torch" ? BRAND.yellow : "none"}
                strokeWidth={1.5} />
              {flashMode !== "off" && (
                <span style={{ fontSize:8, color: BRAND.yellow, fontWeight:700, letterSpacing:0.5 }}>
        ...
        <div style={{ position:"absolute", inset:0, overflow:"hidden" }}>
          ...
          <canvas ref={canvasRef} width={800} height={800}
            style={{ width:"100%", height:"100%", objectFit:"cover", display:"block",
              background: "transparent" }} />
          {/* Filter overlay: CSS filter applied over the native camera layer */}
          {isNative.current && currentFilter && currentFilter.css !== "none" && (
            <div style={{ position:"absolute", inset:0, pointerEvents:"none",
              mixBlendMode:"multiply", filter: currentFilter.css,
        ...
        {showFilters && camState === CAM_STATE.GRANTED && !captured && (
          <FilterCarousel filters={FILTERS} activeId={activeFilter}
            onSelect={setActiveFilter} videoRef={videoRef} isNative={isNative.current} />
        )}
