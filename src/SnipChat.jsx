      ...
      <div style={{ flex:1, overflowY:"auto", padding:"0 12px 24px",
        ...
        {snaps.map(snap => (
          <div key={snap.id} style={{ aspectRatio:"1", borderRadius:6, overflow:"hidden", position:"relative" }}>
            {snap.type === "video"
              ? <video
                  src={snap.media.startsWith("file://") && window.Capacitor?.convertFileSrc
                    ? window.Capacitor.convertFileSrc(snap.media)
                    : snap.media}
...
export default function SnipChat() {
  ...
  const startCamera = useCallback(async (mode) => {
    ...
    try {
      if (isNative.current) {
        // Force Android to show camera + microphone permission dialogs by calling
        // getUserMedia before CameraPreview starts. This is the only reliable way
        // to get both prompts — CameraPreview.start() only asks for camera, never mic.
        // We immediately stop the stream after; we just need the permission grants.
        try {
          const tempStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
          tempStream.getTracks().forEach(t => t.stop());
        } catch (e) {
          // If denied, bail
          if (e.name === "NotAllowedError") { setCamState(CAM_STATE.DENIED); return; }
          // Other errors (NotFoundError etc) — still try CameraPreview
        }
  ...
  // Decodes a JPEG base64 string onto an offscreen canvas, applying correct rotation
  // and the active CSS filter, then returns a PNG data URL.
  const processNativePhoto = (base64jpeg, filterCss, isFront) => new Promise(resolve => {
    const img = new Image();
    img.onload = () => {
      const c = document.createElement("canvas");
      // Always render portrait: if image is landscape from sensor, rotate it
      const w = img.naturalWidth, h = img.naturalHeight;
      const portrait = h >= w;
      c.width  = portrait ? w : h;
      c.height = portrait ? h : w;
      const ctx = c.getContext("2d");
      if (!portrait) {
        // sensor gave us landscape — rotate 90° so it stands up
  ...
  const startRecording = useCallback(async () => {
    ...
    if (isNative.current) {
      // Native: use CameraPreview built-in recording
      try {
        ...
        {captured && !editing && (
          <div style={{ position:"absolute", inset:0, zIndex:50, background:"#000",
            ...
            <div style={{ flex:1, position:"relative", overflow:"hidden" }}>
              {captured.type === "video"
                ? <video
                    src={isNative.current && captured.media.startsWith("file://")
                      ? window.Capacitor.convertFileSrc(captured.media)
                      : captured.media}
