...
export default function SnipChat() {
  ...
  const startCamera = useCallback(async (mode) => {
    ...
    try {
      if (isNative.current) {
        // Request camera + microphone + storage all upfront before touching any API.
        // Uses the native Permissions API directly so it works regardless of plugin version.
        try {
          const results = await Promise.all([
            navigator.permissions.query({ name: "camera" }),
            navigator.permissions.query({ name: "microphone" }),
          ]);
          // On Android, querying is enough to trigger the system prompt.
          // If either is explicitly denied, bail out.
          if (results.some(r => r.state === "denied")) {
            setCamState(CAM_STATE.DENIED); return;
          }
        } catch (_) {} // permissions API unavailable — CameraPreview will prompt natively
